...
def iteratively_resolve(
    ...
    Returns:
        {
          ...
          "final_overlaps": list,        # 最后一次 check_overlaps 的结果
        ...
        if not moves:
            if verbose:
                print(f"[resolver] iter {iteration}: {len(overlaps)} overlap(s) "
                      f"but algorithm produced no moves — giving up.")
            return {
                "converged": False,
    ...
    # 跑满 max_iterations 仍未收敛
    final_overlaps = check_overlaps(scene_data)
    if verbose:
        print(f"[resolver] WARNING: reached max_iterations={max_iterations}, "
              f"{len(final_overlaps)} residual overlap(s).")
    return {
        "converged": not final_overlaps,
