...
def compute_normalized_props(freq_ghz: float) -> tuple:
    """
    计算并 min-max 归一化所有材质在给定频率下的电磁属性。

    Returns:
        normalized:  {材质名 → {"eps_r", "sigma", "eps_r_norm", "sigma_norm"}}
        norm_params: {"freq_ghz", "eps_min", "eps_max", "sigma_min", "sigma_max"}
    """
    ...
    sigma_min, sigma_max = min(sigma_vals), max(sigma_vals)
    ...
    for mat, p in props.items():
        ...
        sigma_norm = (
            (p["sigma"] - sigma_min) / (sigma_max - sigma_min)
            if sigma_max > sigma_min else 0.0
        )
        normalized[mat] = {
            "eps_r":      p["eps_r"],
            "sigma":      p["sigma"],
